...
def compute_cluster_feat_df(modis_df, gfs_dict, clust_thresh):
    ...
    df_dict['x'] = []
    df_dict['y'] = []
    for name in gfs_dict.keys():
        if name not in ["lats", "lons", "days"]:
            df_dict[name] = []
    clust_df = cluster_fires(modis_df, clust_thresh, return_df=True)
