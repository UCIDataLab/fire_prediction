...
from .pipeline_helper import check_spanning_file
...
class GridDatasetGeneration(luigi.Task):
    ...
    def requires(self):
        weather_task = WeatherGridGeneration(data_dir=self.data_dir, start_date=self.start_date, 
                end_date=self.end_date, resolution=self.resolution, bounding_box_name=self.bounding_box_name, 
                fill_method=self.fill_method, rain_offset=self.rain_offset, use_era=self.use_era)
    ...
    def output(self):
        ...
        if self.use_era:
            fn = 'grid_ds_era_%s_modis_%s_%s_%s_%s_%droff_%dk.nc' % (self.resolution, self.bounding_box_name, 
                    self.start_date, self.end_date, self.fill_method, self.rain_offset, self.forecast_horizon) 
        else:
            fn = 'grid_ds_gfs_%s_modis_%s_%s_%s_%s_%droff_%dk.nc' % (self.resolution, self.bounding_box_name, 
                    self.start_date, self.end_date, self.fill_method, self.rain_offset, self.forecast_horizon) 
...
def grid_dataset_span_check_func(test_fn, cur_fn):
    fmt = 'grid_ds_gfs_{resolution}_modis_{bb_name}_{start_date}_{end_date}_{fill}_{y_mem}y_{x_mem}x_{active}a_{horizon}k.nc'
