...
def build_measurement_sel():
    ...
    humidity = GribSelection('humidity', np.uint8).add_selection(name='Surface air relative humidity').add_selection(name='2 metre relative humidity').add_selection(name='Relative humidity', level=2)
    ...
    cape18000 = GribSelection('cape_18000', np.int16).add_selection(name='Convective available potential energy', level=18000)
    ...
    soilm = GribSelection('soil_moisture_content', np.float32).add_selection(name='Volumetric soil moisture content', level=0)
    ...
    shortwrad = GribSelection('short_wave_rad_flux', np.float32).add_selection(name='Downward short-wave radiation flux')
