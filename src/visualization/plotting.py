    ...
    for j, (results, t) in enumerate(results_list):
        for i, metric in enumerate(metrics_):
            ...
            for k, v in sorted(results.items()):
                x_values = range(1, len(results[k]) + 1)
                y = list(map(lambda x: metric(*flat(x)), results[k]))
                ax.plot(x_values, y, "s--", label=k, linewidth=2)

                out[t][k][metric.__name__] = (x_values, y)
...
def plot_results_grid_error_bars(results_list, t_k_arr, metrics_):
    # fig = plt.figure()
    f, axs = plt.subplots(len(metrics_), len(results_list), sharey='row')
    out = recursive_defaultdict()
    for j, (results, t) in enumerate(results_list):
        for i, metric in enumerate(metrics_):
            ...
            for k, v in sorted(results.items()):
                ...
                ax.errorbar(x_values, y, yerr=None, fmt="s--", label=k, linewidth=2)
